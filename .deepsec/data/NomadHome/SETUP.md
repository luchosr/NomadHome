# Agent setup for `NomadHome`

This is a deepsec scanning workspace. Project `NomadHome` was just registered
(target: `..`). Setup is incomplete — `data/NomadHome/INFO.md`
...
2. **Fill in `data/NomadHome/INFO.md`.** It's auto-injected into the AI
...
```bash
pnpm deepsec scan    --project-id NomadHome
pnpm deepsec process --project-id NomadHome
```
