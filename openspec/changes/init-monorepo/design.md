...
- **pnpm pinned to 9.15.9 (not 11.x); CI on Node 20.19 + Corepack.** Two problems pushed off pnpm 11: (a) it requires Node ≥ 22.13, which conflicts with the project's documented Node ≥ 20.19 floor (`project.md` §4); (b) pnpm 10+ gates dependency build scripts behind an approval list (`ERR_PNPM_IGNORED_BUILDS`) that was not honored from either `package.json` `pnpm.onlyBuiltDependencies` or `pnpm-workspace.yaml` on a cold CI install — it passed locally only because the store was already warm. pnpm 9 has no such gate (it builds dependency scripts by default) and runs on Node 20.19, so the toolchain matches the documented floor. CI uses Corepack (bundled with Node) to provide pnpm from the `packageManager` field — `pnpm/action-setup`'s self-installer ignores the `setup-node` Node, so Corepack is more reliable. `cache: pnpm` was dropped (it requires pnpm before `setup-node`); store caching is a follow-up. `project.md` §4 locks "pnpm with workspaces" without a version, so this stays in spec.
