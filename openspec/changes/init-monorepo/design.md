...
- **CI runs on Node 22, `setup-node` before `pnpm/action-setup`.** pnpm 11.1.2 requires Node ≥ 22.13, so `pnpm/action-setup` must self-install on Node 22, not the runner's default (20.x). Node 22 still satisfies the project's `engines.node >= 20.19.0` app-runtime floor (the toolchain needs newer than the runtime). `cache: pnpm` was dropped because it requires pnpm to exist before `setup-node`, which conflicts with this ordering; store caching is a follow-up.
